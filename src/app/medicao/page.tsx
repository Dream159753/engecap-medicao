...
export default function MedicaoScreen() {
  ...
  const adicionarMedicao = (servicoLiberado: ServicoLiberado) => {
    ...
    const novo: MedicaoItem = {
      ...
      secao: servicoLiberado.secao,
      andar: servicoLiberado.andar,
      servico: servicoLiberado.servico,
      quantidade: 0,
      valorUnitario: servicoLiberado.servico.includes("Forma") ? 85.50 : 32.00,
      total: 0
    };
  ...
  const atualizarQuantidade = (id: number, quantidade: number) => {
    setMedicoes(medicoes.map(m => m.id === id ? { ...m, quantidade, total: quantidade * m.valorUnitario } : m));
  };
  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-white rounded-2xl shadow p-8">
        ...
        <div className="flex gap-4 mb-8">
          <div className="flex-1">
            ...
            <div className="flex gap-3">
              <input type="text" value={chapaInput} onChange={(e) => setChapaInput(e.target.value)} placeholder="Ex: 01" className="flex-1 border rounded-lg px-4 py-3" />
        ...
        {/* Serviços Liberados */}
        <h3 className="font-semibold mb-4">Serviços Liberados para Lançamento</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {servicosLiberados.filter(s => s.liberado).map(s => (
            <button 
              key={s.id}
              onClick={() => adicionarMedicao(s)}
              className="border p-6 rounded-xl hover:bg-gray-50 text-left"
            >
        ...
        <div className="mt-10 border-t pt-6 flex justify-between items-center">
          ...
          <button className="bg-green-600 text-white px-12 py-4 rounded-2xl font-semibold text-lg">Finalizar Medição</button>
        </div>
      </div>
    </div>
  );
